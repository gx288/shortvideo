...
# Worksheet list (modify this list as needed)
WORKSHEET_LIST = [
    "Phòng mạch",
    #"Sheet2",
    #"Sheet3",
...
for worksheet_name in WORKSHEET_LIST:
    ...
    for i, row in enumerate(rows):
        ...
        if len(row) > 7 and (not row[7] or row[7].strip() == ''):
            unprocessed_rows.append((i + 1, row))  # Store row number (1-based) and row data

    if not unprocessed_rows:
        print(f"  No unprocessed rows found in '{worksheet_name}'. Moving to next worksheet.")
        continue

    # Process each unprocessed row in the current worksheet
    for row_num, selected_row in unprocessed_rows:
        print(f"  Processing row {row_num} in worksheet '{worksheet_name}'...")
        ...
        with open(os.path.join(output_dir, f"clean_title_{worksheet_name}_{row_num}.txt"), "w") as f:
            f.write(clean_title)
        ...
        try:
            ...
            audio_path = os.path.join(output_dir, f"voiceover_{worksheet_name}_{row_num}.mp3")
            with open(audio_path, "wb") as out:
                out.write(response.audio_content)
            print(f"  Saved audio at: {audio_path}")
        except Exception as e:
            print(f"  Error creating audio: {e}. Skipping row {row_num}.")
            continue

        # Cut audio to max 55s using ffmpeg
        try:
            temp_audio = os.path.join(output_dir, f"temp_voiceover_{worksheet_name}_{row_num}.mp3")
            subprocess.run([
        ...
        title_image_path = os.path.join(output_dir, f"title_image_{worksheet_name}_{row_num}.jpg")
        ...
        def create_video(image_paths, audio_path, output_path):
            ...
            try:
                ...
                print(f"  Error loading audio {audio_path}: {e}. Skipping row {row_num}.")
                return
            ...
            if not clips:
                print(f"  Error: No valid clips to create video for row {row_num}. Skipping.")
                return
            ...
            try:
                ...
                print(f"  Error saving video: {e}. Skipping row {row_num}.")
                return

        output_video_path = os.path.join(output_dir, f"output_video_{worksheet_name}_{clean_title}_{row_num}.mp4")
        ...
        if os.path.exists(output_video_path):
            ...
            print(f"Failed to create video for row {row_num} in worksheet '{worksheet_name}'.")
            continue
