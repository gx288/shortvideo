...
for worksheet_name in WORKSHEET_LIST:
    ...
    for i, row in enumerate(rows):
        ...
        if len(row) > 7 and (not row[7] or row[7].strip() == ''):
            ...
            # Stage 4: Download images với filter clipart
            def download_images_with_icrawler(keyword, num_images, output_dir):
                print("Stage 4: Attempting to download clipart images...")
                ...
                try:
                    ...
                    # Tạo crawler với filter cho clipart
                    ...
                    # Thêm filter cho clipart
                    filters = dict(
                        type='clipart',  # Lọc chỉ lấy clipart
                        size='large',    # Kích thước lớn
                        color='transparent'  # Ưu tiên ảnh có nền trong suốt
                    )
                    ...
                    print(f" Crawled clipart images for keyword: {keyword}")
                ...
                if len(downloaded_files) < 2:
                    print(" Warning: Not enough clipart images downloaded. Using fallback.")
                    return [title_image_path] * max(2, num_images)
                ...
                for full_path in downloaded_files[:num_images]:
                    try:
                        ...
                        final_image = Image.new("RGB", (720, 1280), (255, 255, 255))  # Nền trắng cho clipart
                ...
                if not image_paths:
                    print(" Warning: No valid clipart images processed. Using fallback.")
                    return [title_image_path] * max(2, num_images)
            ...
            print(f" Retrieved {len(additional_images)} clipart images")
