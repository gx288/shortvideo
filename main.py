...
# Process one video from the first unprocessed row found
for worksheet_name in WORKSHEET_LIST:
    ...
    if selected_row:
        print(f"  Processing row {selected_row_num} in worksheet '{worksheet_name}'...")
        break  # Found an unprocessed row, exit the worksheet loop
else:
    print("No unprocessed rows found in any worksheet. Exiting.")
    exit(0)
